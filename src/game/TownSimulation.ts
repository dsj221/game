...
import { influenceFor, happinessFactors, buildingDistance, influenceRules, operationalIds } from "../systems/buildingInfluence.ts";
...
export function metrics(
  ...
  const environment = clamp(
    48 +
      local.reduce((n, b) => n + (b.paused ? 0 : (cfg(b)?.environment || 0) * b.level), 0) -
  ...
  const shopFood = local.reduce((sum,b) => sum + (["food","bread"].includes(cfg(b)?.sells || "") ? facilities[b.id]?.stock || 0 : 0), 0);
...
function assignHomesAndJobs(buildings: Building[], npcs: Npc[]) {
  ...
  for (const n of npcs) {
    ...
    const h = homes.find((b) => b.id === n.home && b.world === n.world);
    if (!h || (homeUse.get(h.id) || 0) >= capacity(h)) n.home = "";
    else homeUse.set(h.id, (homeUse.get(h.id) || 0) + 1);
    const job = jobs.find((b) => b.id === n.workplace && b.world === n.world);
    if (!job || (jobUse.get(job.id) || 0) >= slots(job)) n.workplace = "";
  ...
  for (const job of jobs.filter(b => !(jobUse.get(b.id) || 0))) {
    const worker = npcs.find(n => n.modelType === "villager" && n.world === job.world && (jobUse.get(n.workplace) || 0) > 1);
    if (worker) {
  ...
  for (const n of npcs) {
    ...
    if (!n.workplace) {
      const home = homes.find((b) => b.id === n.home);
      const j = jobs
        .filter(
          (b) => b.world === n.world && (jobUse.get(b.id) || 0) < slots(b),
        )
        .sort((a, b) => {
          const rank = (x: Building) =>
            cfg(x)?.sells ? 0 : cfg(x)?.output ? 1 : 2;
          return (
            ((jobUse.get(a.id) || 0) - (jobUse.get(b.id) || 0)) * 10 +
            rank(a) - rank(b) +
            (home
              ? (Math.abs(a.x - home.x) +
                  Math.abs(a.z - home.z) -
    ...
    const work = jobs.find((b) => b.id === n.workplace);
...
function eventStep(t: TownState, tick: number) {
  t.events = t.events
    .map((e) => ({ ...e, remaining: e.remaining - 1 }))
    .filter((e) => e.remaining > 0);
  if (tick < t.nextEvent) return;
  t.seed = (Math.imul(t.seed, 1664525) + 1013904223) >>> 0;
  const types = ["rain", "festival", "flu"] as const;
  const type = types[t.seed % 3],
    duration = 65;
  const info = {
    rain: ["连绵春雨", "雨水让田间工作慢了一些：农业产量 −20%，持续 65 秒。"],
    festival: [
      "周末小庆典",
      "游客带来订单：消费意愿提高，成交价格 +30%，持续 65 秒。",
    ],
    flu: [
      "换季的喷嚏",
      "轻微流感影响健康和效率；有员工的诊所可减轻影响，持续 65 秒。",
    ],
  }[type];
  t.events = [
    {
  ...
  t.eventHistory = [...t.eventHistory.slice(-19), type];
  t.nextEvent = tick + 145 + (t.seed % 50);
  note(t, info[0], "event", tick);
}
...
export function simulateTown(input: SimInput): SimOutput {
  const t: TownState = structuredClone(input.town),
    npcs: Npc[] = structuredClone(input.npcs),
    bag = { ...input.bag },
  ...
  const rain =
      t.events.some((e) => e.type === "rain") || input.weather.includes("rain"),
    festival = t.events.some((e) => e.type === "festival"),
    flu = t.events.some((e) => e.type === "flu");
  const offline = new Set(input.offline || []);
  const activeIds = operationalIds(buildings,npcs,defs,input.offline,!!input.tiles);
  for(const b of buildings)if(cfg(b)?.jobs&&!onShift(t.minute,t.facilities[b.id],!!cfg(b)?.sells))activeIds.delete(b.id);
  ...
  for (const b of buildings) {
    ...
    const staff = npcs.filter(
      (n) => n.workplace === b.id && n.modelType === "villager",
    );
    ...
    f.efficiency =
      ...
      (connections.has(b.id) || ["住宅", "公共", "装饰"].includes(c.category)
        ? 1
        : 0.5) *
      influenceFor(b, buildings, defs, activeIds).efficiency *
      (flu && !influenceFor(b, buildings, defs, activeIds).health ? 0.8 : 1) *
    ...
    if (c.capacity) {
      f.status = "住宅";
      for (const n of npcs.filter((n) => n.home === b.id)) {
    ...
    if (!c.output) {
      if(c.jobs&&!shift){f.status='休息中';f.efficiency=0;continue;}
    ...
    if (
      Object.entries(c.recipe || {}).some(([r, n]) => bag[r as Resource] < n)
    ) {
      f.status = "缺少原料";
      continue;
    }
    f.status = staff.length < slots(b) ? "员工不足" : "生产中";
    f.progress += f.efficiency / (c.cycle || 30);
    if (f.progress >= 1) {
      f.progress -= 1;
      ...
      for (const [r, n] of Object.entries(c.output)) {
        const quantity = n * b.level * (b.type === "farm" && rain ? 0.8 : 1) * (['food','wheat'].includes(r)?bonuses.food:1);
        bag[r as Resource] += quantity;
        addResource(t.ledger.produced, r as Resource, quantity);
        f.produced += quantity;
        pulse(
          b,
          `+${Number(quantity.toFixed(1))} ${{ wood: "木材", stone: "石材", food: "食物", wheat: "小麦", flour: "面粉", bread: "面包", furniture: "家具", iron: "铁矿", redstone: "红石" }[r as Resource]}`,
        );
      }
    }
  }
  const shops = buildings.filter((b) => cfg(b)?.sells && activeIds.has(b.id)),
    parks = buildings.filter((b) => influenceRules[b.type]?.leisure && activeIds.has(b.id));
  for (const [index, n] of npcs.entries()) {
    ...
    const hasClinic = !!residence && influenceFor(residence,buildings,defs,activeIds).health;
    const localParks = parks.filter(b=>b.world===n.world && (!residence || buildingDistance(residence,b)<=(influenceRules[b.type]?.radius||3)));
    n.health = clamp(
      (n.health ?? 90) +
        (hasClinic ? 0.035 : 0) -
    ...
    const workplace = buildings.find(b=>b.id===n.workplace);
    const shopShift = !!workplace && onShift(t.minute,t.facilities[workplace.id],!!cfg(workplace)?.sells);
    const shop = shops
      .filter(
        (b) =>
          b.world === n.world &&
          ...
          (festival || (foodNeed && ["food","bread"].includes(cfg(b)?.sells || "")) || (funNeed && ['cafe','tea_house'].includes(b.type)) || (shoppingNeed && cfg(b)?.sells === "furniture")),
      )
      .sort((a, b) => {
    ...
    let bought = false;
    if(input.tiles){
      const leisure=t.minute>=1080&&t.minute<1260?localParks[0]:undefined;
      // Keep shop staff at the counter throughout opening hours.
      const destination=shopShift&&cfg(workplace!)?.sells?workplace:(shop&&(foodNeed||funNeed||shoppingNeed||festival))?shop:shopShift?workplace:leisure||residence;
    ...
    if (
      ...
      const c = cfg(shop)!,
        ...
        m = metrics(buildings, npcs, bag, t.facilities);
      const scarcity =
        c.sells === "food" || c.sells === "bread"
          ? 1 + clamp(1 - m.foodDays, 0, 0.2)
          : 1;
      const price = Math.round(
        (c.price || 12) * f.priceFactor * (festival ? 1.3 : 1) * scarcity,
      );
      if ((n.wallet || 0) >= price && f.priceFactor <= 1.5) {
        ...
        n.state = "购物消费";
        n.recent = `在${defs[shop.type].name}买到了${c.sells === "food" ? "新鲜食物" : c.sells === "bread" ? "面包" : "心仪的家具"}，花了 ${price} 金币。`;
        ...
        if (['cafe','tea_house'].includes(shop.type) || festival)
          n.needs.fun = clamp(n.needs.fun - 35);
    ...
    if (n.needs.food > 75 && bag.food >= 1 && tick % 12 === index % 12) {
      bag.food -= 1;
      n.needs.food = clamp(n.needs.food - 35);
      addResource(t.ledger.consumed, "food", 1);
      n.recent = "在社区仓库领到一份食物，希望街角很快有商店。";
    }
    if (!bought && tick - (n.lastPurchase ?? -100) > 12) {
      if (shopShift) {
        n.state='正在工作';n.destination=n.workplace;
        ...
        if (!input.tiles || n.arrivedAt===park.id) n.needs.fun = clamp(n.needs.fun - 0.9);
    ...
    const target = clamp(Object.values(happinessFactors(n,buildings,defs,activeIds,bonuses.happiness)).reduce((a,b)=>a+b,0));
    n.happiness = clamp(
