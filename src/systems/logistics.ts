  ...
  for (const r of Object.keys(bag) as Resource[]) {
    ...
    if (delta > 1e-8) {
      for (const s of warehouses) {
        const n = Math.min(delta, Math.max(0, s.capacity - sum(s.inside)));
        deposit(s, r, n);
        delta -= n;
      }
      if (delta > 0 && warehouses[0]) deposit(warehouses[0], r, delta);
    } else if (delta < -1e-8)
      for (const s of warehouses) {
        const used = takeStock(s, r, -delta);
        delta += used;
        if (delta >= -1e-8) break;
      }
  }
...
export function logisticsStep(
  ...
  for (const s of warehouses.filter((s) => s.active)) {
    const b = buildings.find((b) => b.id === s.id),
      count = s.salvage ? 1 : Math.min(6, (b?.level || 1) * 2),
      entry = ends(s)[0];
    if (!entry) continue;
  ...
  for (const h of order) {
    ...
    if (!h.route.length && goals.some((p) => key(p) === key(h.position))) {
      if (!h.loaded) {
        ...
        h.amount = takeStock(source, h.resource, h.amount);
        if (h.amount <= 0) {
          clear(h);
          continue;
        }
  ...
  for (const h of order.filter(
    (h) => !h.destination && l.stores[h.home]?.active,
  )) {
    ...
    const assign = (
      ...
      h.destination = target.id;
      h.resource = r;
      h.amount = Math.min(h.cart ? 8 : 4, amount);
      h.loaded = false;
      h.route = route;
      h.progress = 0;
      h.status = "前往取货";
    ...
    for (const { s, wanted } of urgent) {
      if (assigned) break;
      for (const [r, n] of Object.entries(wanted) as [Resource, number][]) {
        const need = Math.min(
        ...
        const sources = Object.values(l.stores)
          ...
          .sort(
            (a, b) =>
              Number(b.warehouse) - Number(a.warehouse) ||
